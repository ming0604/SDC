...
def draw_bounding_boxes(image_path, label_path, image_width, image_height):
...
def main():
    ...
    for i in range(len(radar_images)):
        file_name = os.path.splitext(radar_images[i])[0]
        lebel_file_name = file_name + '.txt'
        image_path = os.path.join(yolo_images_path,radar_images[i])
        label_path = os.path.join(yolo_labels_path,lebel_file_name)
        draw_bounding_boxes(image_path, label_path, image_width, image_height)
